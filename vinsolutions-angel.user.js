...
// @updateURL    https://raw.githubusercontent.com/Angeltorx/Botones-crm-angel/blob/main/vinsolutions-angel.user.js
// @downloadURL  https://raw.githubusercontent.com/Angeltorx/Botones-crm-angel/blob/main/vinsolutions-angel.user.js
// ==/UserScript==
...
(function () {
  ...
  console.log(
    `${SCRIPT_PREFIX}: Script INICIANDO en -> ${window.location.href}`
  );
  ...
  if (
    ...
    currentPath.includes(
      "/CarDashboard/Pages/LeadManagement/ActiveLeads_WorkList.aspx"
    ) ||
    currentPath.includes("/CarDashboard/Pages/CRM/ActiveLeads.aspx")
  ) {
    console.log(
      `${SCRIPT_PREFIX} [MainPanel]: Detectada URL de panel principal.`
    );
    const LEFT_PANEL_VIEW_ITEM_LINK_CLASS = "viewitemlink";
    const LEFT_PANEL_ID_EXTRACTION_REGEX =
      /top\.viewItemGCID\((\d+),\s*(\d+)\)/;
        ...
        if (match && match[1] && match[2]) {
          ...
          try {
            if (typeof top.OpenWindow === "function")
              top.OpenWindow(logCallUrl, "LogCallEdit");
            else
              window.open(
                logCallUrl,
                "LogCallEdit",
                "width=800,height=650,resizable=yes,scrollbars=yes"
              );
          } catch (e) {
            window.open(
              logCallUrl,
    ...
    function attachListenersToLinks_LeftPanel() {
      document
        .querySelectorAll(`a.${LEFT_PANEL_VIEW_ITEM_LINK_CLASS}`)
        .forEach((link) => {
    ...
    const observer = new MutationObserver(() => {
      ...
      window.leftPanelDebounce = setTimeout(
  ...
  // =================================================================================
    ...
    currentPath.includes(
      "/CarDashboard/Pages/LeadManagement/LogCallV2/LogCallV2.aspx"
    )
  ) {
    ...
    function resaltarContenido(elementos, regex, claseCss, dataAttribute) {
      elementos.forEach((elem) => {
        ...
        const nuevoHTML = originalHTML.replace(
          regex,
          (match) => `<span class="${claseCss}">${match}</span>`
        );
    ...
    function runAllPopupEnhancements() {
      ...
      if (customerDataContainer) {
        const nameElement = customerDataContainer.querySelector(
          'h4[data-bind="text: fullName"]'
        );
        if (nameElement) {
          const customerName = nameElement.textContent.trim();
          if (customerName && customerName.toLowerCase() !== "name unknown")
            addCopyIcon(nameElement, customerName);
        }
        customerDataContainer
          .querySelectorAll('span[data-bind*="maskPhoneNumber"]')
          .forEach((span) => {
      ...
      const notasDesc = document.querySelectorAll(
        'div#NotesAndHistory span[data-bind="text: Description"]'
      );
      const notasNombres = document.querySelectorAll(
        'div#NotesAndHistory span[data-bind*="AssociatedUserFullName"]'
      );
      resaltarContenido(
        notasDesc,
        regexPalabras,
        "resaltado-clave-vinsol",
        "resaltadoClave"
      );
      resaltarContenido(
        notasDesc,
        regexNombres,
        "resaltado-nombre-vinsol",
        "resaltadoNombre"
      );
      resaltarContenido(
        notasNombres,
